...
class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.err) return (
      <div style={{ minHeight:"100dvh", background:"#0E0F14", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
        <div style={{ maxWidth:600, background:"rgba(220,38,38,0.08)", border:"1px solid rgba(220,38,38,0.3)", borderRadius:14, padding:28 }}>
          <div style={{ fontSize:11, fontWeight:800, color:"#DC2626", letterSpacing:2, textTransform:"uppercase", marginBottom:12 }}>Render Error</div>
          <pre style={{ fontSize:12, color:"#F2F4F8", whiteSpace:"pre-wrap", lineHeight:1.5 }}>{this.state.err?.message}</pre>
          <button onClick={() => this.setState({ err: null })} style={{ marginTop:16, padding:"10px 20px", background:"#CCFF00", color:"#15171E", fontWeight:800, border:"none", borderRadius:9, cursor:"pointer", fontSize:12 }}>Retry</button>
        </div>
      </div>
    );
...
const TOKENS = {
  ...
  fontBody:    "'Plus Jakarta Sans', system-ui, -apple-system, 'Segoe UI', sans-serif",
...
function Scheduler({ session, profile, w }) {
  ...
  const load = async () => {
    const [schedRes, profRes, ovRes] = await Promise.all([
      supabase.from("schedule").select("id, user_id, date").gte("date", startStr).lte("date", endStr),
      supabase.from("profiles").select("id, name"),
      supabase.from("schedule_overrides").select("date, label, hours, max_slots").gte("date", startStr).lte("date", endStr),
    ]);
    ...
    <div style={{ animation:"fadeUp 0.35s ease" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns: dk ? "repeat(5,1fr)" : "1fr", gap:dk?10:9 }}>
        {days.map((day, di) => {
          ...
          const dayMax = override?.max_slots ?? MAX;
...
function Leads({ session, profile, w }) {
  ...
  const [reqState, setReqState] = useState("idle"); // idle | sending | sent
  const dk = w >= 768;
  const isAdmin = profile?.role === "admin";
  ...
  const requestLeads = async () => {
    if (reqState !== "idle" || isAdmin) return;
    setReqState("sending");
    await supabase.from("lead_requests").insert({ user_id: session.user.id, status: "pending", requested_at: new Date().toISOString() });
    setReqState("sent");
  };
  ...
  return (
    <div style={{ animation:"fadeUp 0.35s ease", display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {/* Request Leads — rep only */}
      {!isAdmin && (
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, padding:"14px 18px", background: reqState === "sent" ? "linear-gradient(135deg,rgba(34,197,94,0.07),rgba(34,197,94,0.02))" : "linear-gradient(135deg,rgba(204,255,0,0.05),rgba(204,255,0,0.01))", border:`1px solid ${reqState === "sent" ? "rgba(34,197,94,0.25)" : "rgba(204,255,0,0.18)"}`, borderRadius:14 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, minWidth:0 }}>
            <div style={{ width:36, height:36, borderRadius:10, background: reqState === "sent" ? "rgba(34,197,94,0.12)" : "rgba(204,255,0,0.09)", border:`1px solid ${reqState === "sent" ? "rgba(34,197,94,0.28)" : "rgba(204,255,0,0.22)"}`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              {reqState === "sent"
                ...
                : <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#CCFF00" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.62 3.38 2 2 0 0 1 3.62 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.6a16 16 0 0 0 6 6l.96-.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21.73 16z"/></svg>
              }
            </div>
            <div style={{ minWidth:0 }}>
              <div style={{ fontSize:13, fontWeight:700, color:"#F2F4F8" }}>{reqState === "sent" ? "Request sent!" : "Need more leads?"}</div>
              <div style={{ fontSize:11, color:"#7E8595", marginTop:2 }}>{reqState === "sent" ? "Admin has been notified and will send leads your way." : "Ping admin and they'll drop fresh leads into your pipeline."}</div>
            </div>
          </div>
          <button onClick={requestLeads} disabled={reqState !== "idle"}
            style={{ flexShrink:0, padding:"9px 16px", background: reqState === "sent" ? "rgba(34,197,94,0.10)" : "#CCFF00", border: reqState === "sent" ? "1px solid rgba(34,197,94,0.28)" : "none", borderRadius:10, color: reqState === "sent" ? "#22C55E" : "#15171E", fontSize:11.5, fontWeight:800, cursor: reqState === "idle" ? "pointer" : "default", letterSpacing:1.2, textTransform:"uppercase", fontFamily:"inherit", transition:"all 0.18s", opacity: reqState === "sending" ? 0.6 : 1 }}>
            {reqState === "sending" ? "Sending…" : reqState === "sent" ? "Sent ✓" : "Request Leads"}
          </button>
        </div>
      )}
        ...
        <div className="dash-card" style={{ padding:dk?"16px 22px":"14px 16px" }}>
          <div style={{ display:"flex", alignItems:"baseline", justifyContent:"space-between", flexWrap:"wrap", gap:14 }}>
            <div>
              ...
              <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:dk?38:32, lineHeight:1, color:"#F2F4F8", letterSpacing:0.5, marginTop:4, fontVariantNumeric:"tabular-nums" }}>{totalForFilter}<span style={{ fontSize:dk?14:12, color:"#5E6376", marginLeft:8, letterSpacing:1.5, fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:600 }}>leads</span></div>
            </div>
                ...
                return (
                  <div key={g.id} style={{ display:"flex", flexDirection: isMe ? "row-reverse" : "row", gap:10, marginTop:g.continued?2:10, alignItems:"flex-end" }}>
                    <div style={{ width:32, height:32, flexShrink:0, visibility: g.continued ? "hidden" : "visible" }}>
                      <div style={{ width:32, height:32, borderRadius:10, background: isMe ? "linear-gradient(135deg,#CCFF00,#88AB00)" : `linear-gradient(135deg,${c},${c}88)`, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Plus Jakarta Sans',sans-serif", fontSize:12.5, fontWeight:900, color:"#15171E", boxShadow: isMe ? "0 4px 14px rgba(204,255,0,0.32), inset 0 1px 0 rgba(255,255,255,0.4)" : `0 3px 10px ${c}40, inset 0 1px 0 rgba(255,255,255,0.3)` }}>
                        {name[0]?.toUpperCase()}
                      </div>
                    </div>
...
function AppInner() {
  ...
  const [mailboxProfiles, setMailboxProfiles] = useState({});

  // Admin mailbox: load + realtime
  useEffect(() => {
    if (profile?.role !== "admin") return;
    const loadRequests = async () => {
      const [rqRes, profRes] = await Promise.all([
        supabase.from("lead_requests").select("*").order("requested_at", { ascending: false }),
        supabase.from("profiles").select("id, name"),
      ]);
      setLeadRequests(rqRes.data ?? []);
      const pm = {};
      for (const p of profRes.data ?? []) pm[p.id] = p.name || "Rep";
      setMailboxProfiles(pm);
    };
    loadRequests();
    const ch = supabase.channel("lead-requests-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: "lead_requests" }, loadRequests)
      .subscribe();
    return () => supabase.removeChannel(ch);
  }, [profile?.role]);

  const markRequestSent = async (id) => {
    await supabase.from("lead_requests").update({ status: "fulfilled" }).eq("id", id);
    setLeadRequests(prev => prev.map(r => r.id === id ? { ...r, status: "fulfilled" } : r));
  };
  ...
  useEffect(() => {
    ...
    const close = () => setShowMailbox(false);
  ...
  const FONT_LINK = "https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Plus+Jakarta+Sans:wght@400;500;600;700;800;900&family=IBM+Plex+Mono:wght@400;500;600;700&display=swap";
          ...
          <div style={{ maxWidth:1300, margin:"0 auto" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:dk?18:14, gap:dk?24:12, animation:"fadeUp 0.5s ease" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:dk?8:6, animation:"fadeUp 0.5s ease 0.06s both" }}>
                ...
                {profile?.role === "admin" && (
                  <div style={{ position:"relative" }}>
                    <button onClick={() => setShowMailbox(v => !v)} className="btn-pill icon-only" aria-label="Lead Requests"
                      style={{ position:"relative" }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.62 3.38 2 2 0 0 1 3.62 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.6a16 16 0 0 0 6 6l.96-.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21.73 16z"/>
                      </svg>
                      {pendingRequests.length > 0 && (
                        <span style={{ position:"absolute", top:-4, right:-4, minWidth:16, height:16, borderRadius:8, background:"#FF3370", color:"#fff", fontSize:9, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center", padding:"0 3px", lineHeight:1 }}>
                    ...
                    {showMailbox && (
                      <div className="glass" onClick={e => e.stopPropagation()}
                        style={{ position:"absolute", top:"calc(100% + 10px)", right:0, width:300, borderRadius:14, padding:0, zIndex:200, boxShadow:"0 24px 60px rgba(0,0,0,0.7)", animation:"popIn 0.18s ease", overflow:"hidden" }}>
                        <div style={{ padding:"14px 16px", borderBottom:"1px solid rgba(255,255,255,0.07)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                          <div style={{ fontSize:9.5, fontWeight:800, color:"#FF3370", letterSpacing:2.5, textTransform:"uppercase" }}>Lead Requests</div>
                          {pendingRequests.length > 0 && (
                            <span style={{ fontSize:9, fontWeight:800, background:"rgba(255,51,112,0.12)", border:"1px solid rgba(255,51,112,0.3)", color:"#FF3370", padding:"2px 7px", borderRadius:6, letterSpacing:1 }}>{pendingRequests.length} pending</span>
                          )}
                        </div>
                        <div style={{ maxHeight:320, overflowY:"auto" }}>
                          {leadRequests.length === 0 ? (
                            <div style={{ padding:"24px 16px", textAlign:"center", color:"#5E6376", fontSize:12, fontWeight:500 }}>No requests yet</div>
                          ) : leadRequests.map(r => {
                            const isPending = r.status === "pending";
                            const when = new Date(r.requested_at);
                            const timeStr = when.toLocaleDateString("en-US", { month:"short", day:"numeric" }) + " · " + when.toLocaleTimeString("en-US", { hour:"numeric", minute:"2-digit" });
                            return (
                              <div key={r.id} style={{ padding:"12px 16px", borderBottom:"1px solid rgba(255,255,255,0.04)", display:"flex", alignItems:"center", gap:10, opacity: isPending ? 1 : 0.45 }}>
                                <div style={{ flex:1, minWidth:0 }}>
                                  <div style={{ fontSize:12.5, fontWeight:700, color:"#F2F4F8", marginBottom:2 }}>{mailboxProfiles[r.user_id] || "Rep"}</div>
                                  <div style={{ fontSize:10.5, color:"#5E6376", fontWeight:500 }}>{timeStr}</div>
                                  {r.message && <div style={{ fontSize:11, color:"#9098A8", marginTop:4, fontStyle:"italic" }}>"{r.message}"</div>}
                                </div>
                                {isPending ? (
                                  <button onClick={() => markRequestSent(r.id)}
                                    style={{ flexShrink:0, padding:"5px 10px", background:"rgba(204,255,0,0.10)", border:"1px solid rgba(204,255,0,0.28)", borderRadius:7, color:"#CCFF00", fontSize:10, fontWeight:800, cursor:"pointer", letterSpacing:1, textTransform:"uppercase", fontFamily:"inherit" }}>
                                    Sent ✓
                                  </button>
                                ) : (
                                  <span style={{ fontSize:9.5, color:"#22C55E", fontWeight:700, letterSpacing:1, textTransform:"uppercase" }}>Done</span>
                                )}
                              </div>
                            );
                          })}
